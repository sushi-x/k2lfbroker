        ...
        public override void Execute()
        {
            try
            {
                ...
                using (DataConnector connector = new DataConnector(this))
                {
                    ...
                    Property[] inputProperties = new Property[method.InputProperties.Count];
                    Property[] returnProperties = new Property[method.ReturnProperties.Count];


                    for (int i = 0; i < method.InputProperties.Count; i++)
                    {
                        inputProperties[i] = serviceObject.Properties[method.InputProperties[i]];
                    }

                    for (int i = 0; i < method.ReturnProperties.Count; i++)
                    {
                        returnProperties[i] = serviceObject.Properties[method.ReturnProperties[i]];
                    }

                    // Execute the Service Object method and return any data.
                    connector.Execute(inputProperties, method.Validation.RequiredProperties, returnProperties, method.MethodParameters, method.Type, serviceObject);
                }
